...
abstract contract Router01 is IRouter01 {
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBoptional = UniswapV2Library.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            ...
            if (amountBoptional <= amountBDesired) {
                require(
                    amountBoptional > amountBMin,
                    "Router: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBoptional);
            } else {
                uint256 amountAoptional = UniswapV2Library.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                require(
                    amountAoptional <= amountADesired,
                    "Router: INSUFFICIENT_A_AMOUNT"
                );
                require(
                    amountAoptional > amountAMin,
                    "Router: INSUFFICIENT_A_AMOUNT"
                );
